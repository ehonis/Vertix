...
export const filterCompletionsByType = (completions: MixerCompletion[]) => {
  ...
  const ropeCompletions = completions.filter(completion => completion.type === CompletionType.ROPE);
...
export const findTwoHighestRopeScores = (ropeCompletions: MixerCompletion[]) => {
  ...
  const highestRouteWithFullCompletion = ropeCompletions
  ...
  const remainingRopeCompletions = ropeCompletions.filter(
    completion => completion.id !== highestRouteWithFullCompletion?.id
  );
  const sortedRemainingRopeCompletions = remainingRopeCompletions.sort(
...
export const findTwoHighestRopeScoresPerClimber = (
  groupedByClimberId: Record<string, MixerCompletion[]>
) => {
  console.log(groupedByClimberId);
  return Object.keys(groupedByClimberId).map(climberId => {
    const ropeCompletions = groupedByClimberId[climberId].filter(
...
export const findThreeHighestBoulderScoresPerClimber = (
  groupedByClimberId: Record<string, MixerCompletion[]>
) => {
  return Object.keys(groupedByClimberId).map(climberId => {
    const boulderCompletions = groupedByClimberId[climberId].filter(
      completion => completion.type === CompletionType.BOULDER
    );
...
export const combineTwoHighestRopeScoresPerClimberWithAttempts = (
  twoHighestRopeScoresPerClimber: { climberId: string; twoHighestRopeScores: MixerCompletion[] }[]
) => {
  return twoHighestRopeScoresPerClimber.map(climber => {
    ...
    return {
      ...
      points: climber.twoHighestRopeScores.reduce((acc, completion) => acc + completion.points, 0),
      attempts: climber.twoHighestRopeScores.reduce(
        (acc, completion) => acc + completion.attempts,
        0
      ),
    };
  });
...
export const combineThreeHighestBoulderScoresPerClimberWithAttempts = (
  threeHighestBoulderScoresPerClimber: {
    climberId: string;
    threeHighestBoulderScores: MixerCompletion[];
  }[]
) => {
  return threeHighestBoulderScoresPerClimber.map(climber => {
    ...
    return {
      climberId: climber.climberId,
      points: climber.threeHighestBoulderScores.reduce(
        (acc, completion) => acc + completion.points,
        0
      ),
      attempts: climber.threeHighestBoulderScores.reduce(
        (acc, completion) => acc + completion.attempts,
        0
      ),
    };
  ...
  divisionStandings.forEach((division, index) => {
    ...
    division.climbers.forEach(climber => {
      ...
      if (climber.originalDivision) {
        const divisionOrder = divisionStandings.findIndex(
          d => d.divisionName === division.divisionName
        );
        const originalDivisionOrder = divisionStandings.findIndex(
          d => d.divisionName === climber.originalDivision
        );
    ...
    for (const climber of division.climbers) {
      ...
      for (const potentialDiv of divisionStandings) {
        const difference = Math.abs(
          climber.finishPlacePoints - potentialDiv.averageFinishPlacePoints
        );
...
const moveClimberBetweenDivisions = (
  ...
  fromDivision.climbers = fromDivision.climbers.filter(
    (c: Climber) => c.climberName !== climber.climberName
  );
...
const adjustDivisions = (originalDivisionStandings: DivisionStanding[]) => {
  ...
  for (let i = 0; i < divisionStandings.length - 1; i++) {
    ...
    for (const climber of climbersToMoveUp) {
      moveClimberBetweenDivisions(
        divisionStandings,
        climber,
        lowerDiv.divisionName,
        higherDiv.divisionName
      );
    }
  }
  ...
  for (let i = 0; i < divisionStandings.length - 1; i++) {
    ...
    if (lowerDiv.climbers.length > 0) {
      const bestLowerDivScore = Math.min(
        ...lowerDiv.climbers.map((c: Climber) => c.finishPlacePoints)
      );
...
const adjustDivisionsWithAverageDownwardMovement = (
  originalDivisionStandings: DivisionStanding[]
) => {
  // Create a deep copy and mark original divisions
  const divisionStandings = JSON.parse(JSON.stringify(originalDivisionStandings));
...
export const calculateStandings = async (compId: string): Promise<StandingsData> => {
  ...
  const overallStandings = Array.from(climberPlaces.entries()).map(([climberName, places]) => {
    const division =
      sortedBoulderScores.find(s => s.climberName === climberName)?.division || "No Division";
    return {
      climberName,
      division,
  ...
  const standingsByDivision = finalStandings.reduce(
    (acc, climber) => {
  ...
  const sortedDivisionStandings = divisionStandings.sort(
    (a, b) => a.averageFinishPlacePoints - b.averageFinishPlacePoints
  );
...
export const calculateStandingsWithAverageDownwardMovement = async (
  compId: string
  ...
  const adjustedDivisionStandings =
    adjustDivisionsWithAverageDownwardMovement(sortedDivisionStandings);
...
export function generateCompetitionCsv(
  ...
  for (const division of originalDivisions) {
    // Add division header
    csvRows.push(
      `"${division.divisionName}",,average = ${division.averageFinishPlacePoints},,,,,,,,,,,,`
    );

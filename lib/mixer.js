...
export function getTopScores(completions, points) {
  ...
  const trueIds = Object.entries(completions).filter(([key, value]) => value === true);
...
export function formatMultipleRoutesNotCalculated(routes) {
  const result = routes.map(route => ({
    name: route.name,
    color: route.color,
    holds: Array.from({ length: route.holds }, (_, index) => ({
      holdNumber: index + 1,
      topRopePoints: 0,
      leadPoints: 0,
    })),

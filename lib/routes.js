...
export async function getRouteById(id) {
  try {
    ...
    console.error(`error finding route with id: ${routeId} with error:${error}`);
  }
}
...
export async function findCommunityGrade(routeId) {
  try {
    ...
    const grades = communityGradeObject
      .map(route => route.grade)
      .filter(grade => grade.toLowerCase() !== "vfeature" && grade.toLowerCase() !== "5.feature");
    const gradeMap = {
      "5.b": 6.0,
      "5.7-": 7.0,
      5.7: 7.2,

...
export function getGradeCounts(ropeCompletions, boulderCompletions) {
  ...
  const boulderTransformedData = Object.keys(boulderGradeCounts).map(grade => ({
    grade,
...
export async function getPieChartCompletionsData(userId) {
  try {
    ...
    const boulderCompletions = routes.filter(route => route.type === "boulder");
...
export async function getLineChartCompletionsData(userId) {
  ...
  completions.forEach(routeCompletion => {
    routeCompletion.completionDate = convertToEST(routeCompletion.completionDate);
  });

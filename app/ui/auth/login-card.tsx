...
export default function LoginCard() {
  ...
  const fieldErr =
    clerkErrors?.fields?.identifier?.message ?? clerkErrors?.fields?.code?.message ?? null;
  ...
  async function onSendCode(e: React.FormEvent) {
    ...
    try {
      ...
      if (createResult.error) {
        setError(createResult.error.message ?? "Failed to start sign-in");
        return;
      }

      // Step 2: send an email code
  ...
  async function onVerifyCode(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      const verifyResult = await signIn.emailCode.verifyCode({ code: otp.trim() });
      ...
      if (signIn.status === "complete") {
        ...
        if (finalizeResult.error) {
          setError(finalizeResult.error.message ?? "Failed to finalize sign-in");
          return;
        }
  ...
  return (
    <section className="glass-card rounded-2xl p-6 sm:p-8 text-white">
      <div>
        <h2 className="font-jost text-xl tracking-tight text-white">Welcome back</h2>
        <p className="mt-1 font-barlow text-sm text-white/50">
          {step === "email" ? "Enter your email to get a code." : "Enter the code we sent."}
        </p>
      </div>
          ...
          <form onSubmit={onSendCode} className="space-y-3">
            <label className="block">
              ...
              <input
                ...
                autoComplete="email"
                inputMode="email"
                className="w-full rounded-xl border border-white/10 bg-white/[0.02] px-3 py-3 font-barlow text-sm text-white outline-none transition placeholder:text-white/25 focus:border-blue-500/60"
              />
            </label>
            <button
              type="submit"
              disabled={!canRequest}
              className="w-full rounded-xl bg-[linear-gradient(135deg,#3b82f6,#8200db)] px-4 py-3 font-barlow text-sm font-semibold text-white transition hover:brightness-110 disabled:opacity-50"
            >
          ...
          <form onSubmit={onVerifyCode} className="space-y-3">
            ...
            <label className="block">
              ...
              <input
                ...
                inputMode="numeric"
                className="w-full rounded-xl border border-white/10 bg-white/[0.02] px-3 py-3 font-barlow text-sm tracking-[0.22em] text-white outline-none transition placeholder:text-white/25 focus:border-blue-500/60"
              />
            </label>

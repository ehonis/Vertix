...
export default function BoulderLeagueSignUpForm({
  divisions,
  user,
  compId,
}: BoulderLeagueSignUpFormProps) {
  ...
  const handleSignUp = async () => {
    if (climberName === "" && !isError) {
      ...
      try {
        const response = await fetch("/api/boulder-league/user/signup", {
          method: "POST",
          body: JSON.stringify({
            userId: user.id,
            climberName,
        ...
        if (response.ok) {
          ...
          showNotification({
            message: `Error in database, could not sign up `,
            color: "red",
          });
        }
      } catch (error) {
  ...
  const handleClimberNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {

...
export default function BoulderLeagueScorer({
  compId,
  climberId,
  boulders,
  boulderCompletions,
}: BoulderLeagueScrollerData) {
  ...
  const [boulderAttempts, setBoulderAttempts] = useState<BoulderState>(() => {
    return boulders.reduce((acc: BoulderState, panel) => {
      acc[panel.id] = 0;
      return acc;
    }, {});
  });
  ...
  const [completions, setCompletions] = useState<CompletionState>(() => {
    ...
    return boulders.reduce((acc: CompletionState, panel) => {
      // Check if this boulder is in the database completions
      // If boulderCompletions is undefined or empty, default to false
      const isCompleted =
        boulderCompletions?.some(completion => completion.boulderId === panel.id) ?? false;
      if (isCompleted) {
        // If boulder is completed, get the attempts from boulderCompletions
        const completion = boulderCompletions.find(c => c.boulderId === panel.id);
    ...
    <>
      ...
      <div className="flex flex-col w-full items-center">
        ...
        <div className="flex justify-between w-3/4 md:w-1/5 text-sm text-gray-400">
          <span className="text-left font-barlow font-bold">{boulders[0].points}</span>
          <span className="text-center font-barlow font-bold">
            {boulders[Math.floor(boulders.length / 2)].points}
          </span>
          <span className="text-right font-barlow font-bold">
            {boulders[boulders.length - 1].points}
          </span>
        </div>
      </div>
    </>

...
type EditUserPopupData = {
  ...
  type: string;
  onCancel: () => void;
  climber: BLClimber | undefined;
  boulderScore: BLBoulderScore | undefined;
...
export default function EditUserPopUp({
    ...
    const getConnectUser = async () => {
      ...
      try {
        const response = await fetch(
          `/api/boulder-league/manager/user/getBLUser?${queryData.toString()}`
        );
        if (!response.ok) {
          console.log("error");
        }
  ...
  const handleSubmit = async () => {
    if (climberName === "") {
      ...
      try {
        const response = await fetch("/api/boulder-league/manager/user/updateUser", {
          method: "POST",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch("/api/boulder-league/manager/user/deleteUser", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const handleNewUserSubmit = async () => {
    if (climberName === "") {
      ...
      try {
        const response = await fetch("/api/boulder-league/manager/user/addNewUser", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(data),
        });

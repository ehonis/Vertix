...
export default function EditDivisionPopUp({
  ...
  const handleUpdateDivision = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/boulder-league/manager/division/updateDivision", {
        method: "POST",
  ...
  const handleNewDivision = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/boulder-league/manager/division/newDivision", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });

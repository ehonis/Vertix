...
export default function NewBoulderPopUp({ onCancel, compId }: NewRoutePopUpProps) {
  ...
  const handleAddMultipleBoulders = async () => {
    ...
    try {
      const response = await fetch("/api/boulder-league/manager/boulder/add-boulders", {
        method: "POST",
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <h2 className="text-xl">W{week} New Boulders</h2>

          {!isBoulderInformation ? (
            <div className="flex flex-col gap-3 items-center">
              {/* if the user picked all routes */}
              <p className="font-normal">Which week are you adding boulders for?</p>
              <div className="flex gap-5 items-center">
                <button
                  className={clsx("rounded-full bg-gray-500 size-8 ", week === 1 && "bg-green-500")}
                  onClick={() => setWeek(1)}
                >
                  1
                </button>
                <button
                  className={clsx("rounded-full bg-gray-500 size-8 ", week === 2 && "bg-green-500")}
                  onClick={() => setWeek(2)}
                >
                  2
                </button>
                <button
                  className={clsx("rounded-full bg-gray-500 size-8 ", week === 3 && "bg-green-500")}
                  onClick={() => setWeek(3)}
                >
                  3
                </button>

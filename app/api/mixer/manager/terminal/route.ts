...
export async function GET(request: NextRequest){
    ...
    const formattedCompletionData = completionData.map((item) => {
        ...
        if(item.type === CompletionType.BOULDER){
            const boulderName = boulderKeyValues.find((boulder) => boulder.boulderId === item.mixerBoulderId)?.boulderName
            return {
                id: createNewId(),
            ...
            const routeName = routeKeyValues.find((route) => route.routeId === item.mixerRouteId)?.routeName
            return {
                id: createNewId(),
                climberId: item.climberId,

    ...
    const result = await prisma.$transaction(
      async tx => {
        ...
        for (const climber of completedClimbers) {
          ...
          if (
            currentHighestGrade &&
            (!user.highestRopeGrade || isGradeHigher(user, currentHighestGrade, "rope"))
          ) {
            console.log(
              `Updating highest rope grade for user ${climber.userId} from ${user.highestRopeGrade || "none"} to ${currentHighestGrade}`
            );

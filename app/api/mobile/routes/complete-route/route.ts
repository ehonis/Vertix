...
import { CompetitionType, RouteType, User } from "@/generated/prisma/client";
import {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { routeId, flash, date, isCompetition, competitionType, competitionId } =
      await req.json();
    ...
    if (hasCompType) {
      if (!Object.values(CompetitionType).includes(competitionType as CompetitionType)) {
        return NextResponse.json({ message: "Invalid competitionType" }, { status: 400 });
      }

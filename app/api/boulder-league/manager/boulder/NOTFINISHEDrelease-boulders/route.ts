...
import { ClimberStatus, Route, RouteType, MixerBoulder, BLBoulder } from "@prisma/client";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { compId, boulders }: { compId: string; boulders: BLBoulder[] } = await req.json();

    // Wrap all database operations in a transaction
    const result = await prisma.$transaction(
      async tx => {
        // Check if competition exists
        const comp = await tx.bLCompetition.findUnique({
          where: {
            id: compId,
          },
        });
        ...
        const completedClimbers = await tx.bLClimber.findMany({
          where: {
        ...
        for (const climber of completedClimbers) {
          ...
          for (const completion of climber.completions) {
            ...
            const actualBoulderId = boulderIdMap.get(completion.boulderId!);
            ...
            if (completion.isComplete) {
              ...
              if (routeInfo) {
                ...
                if (!currentHighestGrade || isGradeHigher(user, newGrade, "boulder")) {
          ...
          if (
            currentHighestGrade &&
            (!user.highestBoulderGrade || isGradeHigher(user, currentHighestGrade, "boulder"))
          ) {
            console.log(
              `Updating highest boulder grade for user ${climber.userId} from ${user.highestBoulderGrade || "none"} to ${currentHighestGrade}`
            );
    ...
    return NextResponse.json(
      { message: "Successfully created routes and processed completions" },
      { status: 200 }
    );
  } catch (error) {

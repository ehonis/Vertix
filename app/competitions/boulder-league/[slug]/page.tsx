...
export default async function page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (user) {
    climberExists = await prisma.bLClimber.findFirst({
      where: {
      ...
      <div className="flex flex-col justify-center items-center gap-8 bg-slate-900 p-10 rounded-md md:w-md w-xs">
        {competition.imageUrl ? (
          ...
          <svg
            ...
            fill="#000000"
            viewBox="0 0 256 256"
            className="size-36 stroke-white fill-white relative z-10 self-center place-self-start"
          >
        ...
        <div className="flex flex-col gap-5">
          {competition.status === CompetitionStatus.UPCOMING && climberExists !== null && (
            <div className="flex flex-col gap-1">
              <p className="text-white font-barlow font-bold text-md  rounded-md p-2 text-center">
                You are already signed up for this competition!
              </p>
              {competition.weekOneStartDate ? (
                <p className="text-white font-barlow font-bold text-md rounded-md p-2 text-center">
                  Come back to this page on{" "}
                  {competition.weekOneStartDate?.toLocaleDateString("en-US")} to start the
                  competition!
                </p>
              ) : (
                <p className="text-white font-barlow font-bold text-xs bg-green-500/25 border-green-500 border rounded-md p-2 text-center">
          ...
          {competition.status === CompetitionStatus.UPCOMING && climberExists === null && (
            <div className="flex flex-col gap-1">
              ...
              {!user ? (
                ...
                <Link
                  href={`/competitions/boulder-league/${compId}/signup`}
                  className="px-2 py-1 text-white bg-green-500 rounded-sm font-barlow font-bold text-center shadow-xl"
                >
                  Sign up
                </Link>
            ...
            <div className="flex flex-col gap-1">
              ...
              {user?.role === "ADMIN" && (
                <div className="flex flex-col gap-2">
                  <Link
                    href={`/competitions/boulder-league/${compId}/scroller`}
                    className="px-2 py-1 text-white bg-green-500 rounded-sm font-barlow font-bold text-center shadow-xl"
                  >
                    Start Competition
                  </Link>
                  <Link
                    href={`/competitions/boulder-league/${compId}/leaderboard`}
                    className="px-2 py-1 text-white bg-green-500 rounded-sm font-barlow font-bold text-center shadow-xl"
                  >
                    Leaderboard
                  </Link>
          ...
          {competition.status === CompetitionStatus.IN_PROGRESS && climberExists !== null && (
            <div className="flex flex-col gap-2">
              {climberExists.climberStatus === ClimberStatus.IN_PROGRESS ? (
                <Link
                  href={`/competitions/boulder-league/${compId}/scroller`}
                  className="px-2 py-1 text-white bg-green-500 rounded-sm font-barlow font-bold text-center shadow-xl"
                >
